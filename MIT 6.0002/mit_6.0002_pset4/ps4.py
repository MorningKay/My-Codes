...
class Patient(object):
    ...
    def update(self):
        ...
        """
        pop = self.get_total_pop()
        bacteria_surv = []
        for bac in self.bacteria:
            if bac.is_killed():
                pass
            else:
                bacteria_surv.append(bac)
        pop = len(bacteria_surv)
        pop_density = pop / self.max_pop
        bacteria_repr = bacteria_surv.copy()
        for bac in bacteria_surv:
            try:
                offspring = bac.reproduce(pop_density)
                bacteria_repr.append(offspring)
            except NoChildException:
                pass
        self.bacteria = bacteria_repr
        return len(self.bacteria)
...
def calc_pop_avg(populations, n):
    ...
    for population in populations:
        total_pop.append(population[n])
    return sum(total_pop) / len(total_pop)
...
def simulation_without_antibiotic(num_bacteria,
    ...
    x_coords = [x for x in range(300+1)]
    y_coords = [calc_pop_avg(populations, n) for n in range(300)]
    y_coords.insert(0, num_bacteria)
...
def calc_pop_std(populations, t):
    ...
    deviation_square = []
    for population in populations:
        deviation_square.append((population[t] - avg_pop)**2)
    deviation_total = sum(deviation_square) / len(deviation_square)
    std = math.sqrt(deviation_total)
    return std
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        if self.on_antibiotic:
            ...
            for bac in bacteria_surv:
                if bac.get_resistant():
                    bacteria_resist.append(bac)
            bacteria_surv = bacteria_resist.copy()
...
# random.seed(0)
# bacteria = [ResistantBacteria(0.3, 0.2, False, 0.8)] * 100
# patient = TreatedPatient(bacteria, 1000)
# print(patient.get_total_pop(), patient.get_resist_pop())
# print("----------")
# for _ in range(10):
#     patient.update()
#     print(patient.get_total_pop(), patient.get_resist_pop())
...
def simulation_with_antibiotic(num_bacteria,
    ...
    x_coords = [x for x in range(400+1)]
    y_coords1 = [calc_pop_avg(populations, n) for n in range(400)]
    y_coords1.insert(0, num_bacteria)
    y_coords2 = [calc_pop_avg(resistant_pops, n) for n in range(400)]
