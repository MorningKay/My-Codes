...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        pixel_matrix = [[p] for p in pixel]
        transformation = matrix_multiply(transformation_matrix, pixel_matrix)
        transformation_tuple = tuple(int(value) for row in transformation for value in row)
        transformation_list.append(transformation_tuple)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    if type(pixel) == int:
        LSB = pixel%(2**num_end_bits)
        return LSB
    else:
        LSBs = tuple(pix%(2**num_end_bits) for pix in pixel)
        return LSBs
...
def reveal_bw_image(filename):
    ...
    hidden_pixels = [extract_end_bits(1, pixel)*255 for pixel in pixels]
    size = img.size
    img.close()
...
def reveal_color_image(filename):
    ...
    img = Image.open(filename)
    pixels = img_to_pix(filename)
    hidden_pixels = [tuple(map(lambda x:int(x*(255/7)), extract_end_bits(3, pixel))) for pixel in pixels]
    size = img.size
    img.close()
...
def main():
    ...
    # im1 = draw_kerb('hidden1.bmp', 'MorningKay')
    im.show()
    print(img_to_pix('hidden1.bmp'))
