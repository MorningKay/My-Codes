...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    decrypt_messages = []
    for i in pads:
        a = ps4b.EncryptedMessage(ciphertext.get_text())
        aplaintext = a.decrypt_message(i)
        decrypt_messages.append(aplaintext.get_text())
    count_true_list = []
    word_list = load_words(WORDLIST_FILENAME)
    for i in decrypt_messages:
        split_word_list = i.split()
        count_true = 0
        for j in split_word_list:
            if (is_word(word_list, j)):
                count_true += 1
        count_true_list.append(count_true)
    max_count_value = max(count_true_list)
    index = max([i for i in range(len(count_true_list)) if count_true_list[i] == max_count_value])
    return ps4b.PlaintextMessage(decrypt_messages[index], pads[index])
...
def decode_story():
    ...
    '''
    decrypt_story = ps4b.EncryptedMessage(get_story_string())
    pad = get_story_pads()
    story = decrypt_message_try_pads(decrypt_story, pad)
